...
std::vector<std::pair<size_t, size_t>> QGramIndex::mergeLists(
  ...
  std::vector<size_t> mergedList;
  std::vector<std::pair<size_t, size_t>> resultList;

  // concatenate all lists and then sort
  for (const auto& list : lists) {
    std::copy(list.begin(), list.end(),
        std::back_insert_iterator<std::vector<size_t>>(mergedList));
  }
  std::sort(mergedList.begin(), mergedList.end());

  // convert the sorted list into pairs of (element, count)
  for (const auto& elm : mergedList) {
    if (resultList.empty() || resultList.back().first != elm) {
      resultList.push_back(std::make_pair(elm, 1));
    } else {
      resultList.back().second++;
    }
  }
...
std::pair<std::vector<Entity>, size_t> QGramIndex::findMatches(
    const std::string& prefix, size_t delta) const {
